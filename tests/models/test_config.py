...
class TestTrainConfigT42PromotedFields:
    ...
    def test_grad_accum_steps_defaults_to_one(self, tmp_path):
    ...
    def test_eval_batch_size_rejects_non_positive_values(self, tmp_path, eval_batch_size):
        """eval_batch_size must be >= 1 when provided; it accepts no 'auto' sentinel."""
        with pytest.raises((ValueError, ValidationError)):
            self._tc(tmp_path, eval_batch_size=eval_batch_size)

    def test_eval_batch_size_defaults_to_none(self, tmp_path):
