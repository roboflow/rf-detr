...
Zones and lines are configured in normalised coordinates and resolved to pixels the first
time a frame arrives, so a stream whose resolution changes mid-run re-resolves cleanly.

Everything here is edge-triggered: an entry is emitted when a track first appears inside a
zone, an exit when it has been absent for a short grace period (boxes flicker across a
boundary far more often than objects do), and an occupancy violation only on the frame the
limit is first exceeded — never once per frame while it stays exceeded.
"""
