...
class RFDETRDataModule(LightningDataModule):
    ...
    def _warn_on_unaligned_batch_sampler(self, batch_sampler: torch.utils.data.Sampler[list[int]]) -> None:
        ...
        """
        # The effective count, not TrainConfig's: Lightning owns accumulation for detection and segmentation, so a
        # caller overriding accumulate_grad_batches on the Trainer would otherwise be checked against a stale number.
        grad_accum_steps = self._effective_grad_accum_steps()
    ...
    def train_dataloader(self) -> DataLoader[Any]:
        ...
        for exactly which kwargs still apply).

        Otherwise, on one process, uses a replacement sampler when the dataset is too small to fill
        ``_MIN_TRAIN_BATCHES`` effective batches (matching legacy behaviour in ``main.py``). In DDP, repeats short
        datasets through :class:`GradAccumAlignedDataset` so that Lightning can safely inject
        ``DistributedSampler`` without discarding the requested sample count. The wrapper also ensures the length is
        an exact multiple of ``effective_batch_size * world_size`` (workaround for
