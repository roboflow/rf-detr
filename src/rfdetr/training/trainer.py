        ...
        if torch.cuda.is_available():
            ...
            if amp_dtype == "bf16":
                if torch.cuda.is_bf16_supported():
                    if not _cuda_supports_native_bf16():
                        emulated_message = (
                            "amp_dtype='bf16' runs bfloat16 through emulation on this GPU, which has no native "
                            "bfloat16 support (pre-Ampere, e.g. T4 or V100), so training is much slower than in fp16. "
                ...
                return "16-mixed"
            # amp_dtype == "auto"
            return "bf16-mixed" if _cuda_supports_native_bf16() else "16-mixed"
