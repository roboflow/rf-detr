...
:meth:`rfdetr.training.module.RFDETRModelModule.on_load_checkpoint`.
"""
...
def convert_legacy_checkpoint(old_path: str, new_path: str) -> None:
    ...
    * ``state_dict`` keys are prefixed with ``"model."`` to match the
      attribute path inside :class:`~rfdetr.training.module.RFDETRModelModule`.
    ...
    * ``legacy_checkpoint_format: True`` is written so
      :meth:`~rfdetr.training.module.RFDETRModelModule.on_load_checkpoint` can
