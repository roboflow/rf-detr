...
- Kornia `Affine` now applies scalar `translate_percent` to both axes and reads scalar `scale` as a fixed range, matching Albumentations configs instead of silently losing horizontal translation or failing during construction.
