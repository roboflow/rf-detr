...
- The decoder's grouped self-attention now reuses the same grouped query tensor as the key instead of computing an identical, independent copy: `TransformerDecoderLayer.forward_post` builds `q = k = with_pos_embed(tgt, query_pos)` and, only during training, regroups `q` via `torch.cat(q.split(...))` and used to regroup `k` the same way from scratch even though `k` was already equal to `q` at that point — `k` now simply aliases the already-regrouped `q`. Outputs and both input and parameter gradients are bit-identical to the previous behavior in every configuration tested (CPU float32, CUDA float16/bfloat16, with/without determinism, batch sizes and group counts matching real training configs), confirmed by a real COCO8-Seg image's full `RFDETRSegNano` training forward producing an identical output hash with and without the change. Isolated microbenchmarks (single decoder self-attention layer, 13 groups, deterministic, median of 31 alternating-order trials, 3 repeated processes) measured 5.44%-5.52% lower latency and one fewer duplicate allocation (2,662,400 bytes eliminated per layer) on CUDA bfloat16/float16; CPU float32 showed a smaller reduction (roughly 2.9%-4.0% across repeated runs on a shared machine). Profiling the same real image's full training forward removed 4 `aten::cat` calls and 5,324,800 bytes of aggregate CPU allocation traffic across the decoder; whole-model forward latency was not consistently faster in single-image runs (dominated by other ops and machine noise), so there is deliberately no end-to-end speed claim here.
