...
- Validation now evaluates **one** model per epoch instead of two: the EMA-averaged weights when `use_ema=True` (the default), the base weights otherwise. The base-model forward pass that `COCOEvalCallback` used to run over every validation batch is gone, which removes one full pass over the validation set per epoch — measured at 11.32 s in one L4 run with a 1978-image validation split and a 222 s epoch, approximately 5.1% in that run; the fraction depends on the validation-to-training ratio and is not a fixed figure. New `TrainConfig.eval_base_model` (default `False`) restores the previous base+EMA comparison. `TrainConfig.eval_ema_only` is deprecated (removal in v2.0): explicit legacy `True` preserves EMA-only evaluation and explicit legacy `False` preserves base-plus-EMA evaluation, with a `FutureWarning`; omit it from new configurations. It still requires `use_ema=True` when true and conflicts with `eval_base_model=True`.
