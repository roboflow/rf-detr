...
- The eager forward pass no longer rebuilds the sine position embedding, the per-level padding masks, or the padded batch tensor on every evaluation call when the batch carries no padding. `nested_tensor_from_tensor_list` already decides, from Python-side shapes alone, whether any image needed padding; it now records that on `NestedTensor.no_padding` (default `False`, so every other construction site keeps the previous conservative behavior) instead of discarding it. For a batch flagged that way the padded tensor is the input itself rather than a fresh allocation plus a full copy, each feature level's mask is built directly as zeros instead of interpolating an all-False mask, and evaluation-mode `PositionEmbeddingSine` serves the embedding from a cache bounded to 8 entries. Its key covers shape, device, layout, and encoding configuration; entering training or applying a device/dtype transformation clears it. Training never populates the cache. Batches that carry real padding, including divisor round-up from `block_size`, are untouched and still read the mask. Outputs are bit-identical.
