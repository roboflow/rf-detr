...
- Fixed `[tflite]` optional dependency group failing to install on Python 3.13: all five `[tflite]` deps are now gated with `; python_version < '3.13'` markers (TensorFlow does not support Python 3.13), and an `ml-dtypes` override restricts its resolution to `python_version < '3.13'` to prevent uv's universal resolver from surfacing a numpy contradiction. The tests group `pytest` cap is widened from `<9` to `<10` to resolve the `pytest==9.0.2` conflict introduced by onnx2tf 2.4.0. ([#1041](https://github.com/roboflow/rf-detr/pull/1041))
