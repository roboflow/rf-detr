...
- Fixed custom Albumentations configs treating `TimeReverse` as a pixel-only transform, which flipped images while leaving boxes and keypoints unchanged. `TimeReverse` now shares the geometric-transform and replay-based keypoint handling used by `HorizontalFlip`. The keypoint safety filter now also disables `SquareSymmetry`, the alias of `D4`, when no left/right `keypoint_flip_pairs` are configured. The default torchvision pipeline is unchanged, as are configs already using the canonical `HorizontalFlip`/`D4` names; configs using the `TimeReverse`/`SquareSymmetry` aliases now get that same handling instead of being silently skipped, including with explicit `keypoint_flip_pairs`.
