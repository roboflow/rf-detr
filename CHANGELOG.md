...
- `RFDETR.predict(shape=...)` — optional `(height, width)` tuple overrides the default inference resolution; useful for matching the resolution used when exporting the model. Both dimensions must be positive integers divisible by the model's effective stride (`patch_size * num_windows`, 14 for the default checkpoints). (closes #682)
