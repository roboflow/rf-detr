...
- `build_trainer` now selects `XLAStrategy` when the accelerator resolves to XLA/TPU — including the default `accelerator="auto"`, once it actually resolves to XLA — and more than one device or node is requested. `XLAAccelerator` pairs only with `SingleDeviceXLAStrategy` or `XLAStrategy`, while Lightning's `strategy="auto"` resolves to DDP as soon as `devices > 1` or `num_nodes > 1`, so asking for more than one TPU chip or host failed with `The XLAAccelerator can only be used with a SingleDeviceXLAStrategy or XLAStrategy, found DDPStrategy` — raised during `Trainer` construction, before any accelerator work, which made a missing capability read as a configuration error. This is the same omission `precision` already needed handling for on this path. Single-device, single-node runs are unaffected, since `"auto"` already resolves to `SingleDeviceXLAStrategy` there, and an explicitly passed `strategy` is never overridden. Keypoint models (`use_grouppose_keypoints=True`) are excluded, since their manual-optimization `find_unused_parameters=True` handling has no validated XLA equivalent, and keep hitting the pre-existing mismatch; segmentation models are not excluded and reach the same guard as plain detection. Verified on a Cloud TPU v6e-4 (`accelerator="tpu", devices=4`): `Trainer` construction went from raising the error above to `STRATEGY: XLAStrategy, ACCELERATOR: XLAAccelerator`, and a 4-chip collective ran successfully. End-to-end training across replicas with the pre-existing `strategy="xla"` workaround (unaffected by this fix, measured 2026-08-24 on an earlier `develop` commit) was separately measured on a v5litepod-4: `devices=4` went from failing in 6.3 s to completing training across real `xmp.spawn` replicas. ([#1058](https://github.com/roboflow/rf-detr/issues/1058))
