...
- Fixed `RFDETR.predict()` returning wrong class names for pretrained COCO checkpoints: raw COCO category IDs (1–90, non-contiguous) were used as 0-indexed offsets into `COCO_CLASS_NAMES`, so e.g. `class_id=18` resolved to `"sheep"` instead of `"dog"`. Predictions from pretrained COCO models now use a sparse ID→name dict built from sorted COCO category IDs. Fine-tuned models with `remap_category_ids=True` are unaffected. Also fixes background class `class_id=90` being labelled `"toothbrush"` instead of `"__background__"`. ([#1005](https://github.com/roboflow/rf-detr/pull/1005), closes [#988](https://github.com/roboflow/rf-detr/issues/988))
