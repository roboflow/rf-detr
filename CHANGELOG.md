...
- The torchvision-native training pipeline no longer resamples twice, through a squared intermediate, on the non-square crop branch. `_build_train_resize_transforms(square=False)` built its scale-jitter branch as short-side resize → `RandomSizedCrop` to a fixed `384x384` → a second `RandomResize` to the requested scales. Every image taking that branch was therefore resampled down to 384 and back up to the target, and the fixed square output discarded the aspect ratio the non-square pipeline exists to preserve before the final resize ever ran. The crop now resizes directly to a randomly chosen target scale, which is what the square path already did and what the Albumentations backend adopted for its own non-square branch — the two backends now express the same recipe again. This changes the augmented pixel distribution for non-square training: crops keep their detail and their aspect ratio instead of arriving pre-squared and twice-resampled. Square training (the released default for every shipped model config) is untouched, as are validation, prediction and export preprocessing.
