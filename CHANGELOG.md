...
- Added `TrainConfig.pack_targets` (default `False`), which concatenates each batch's per-sample target dicts into one tensor per field before they cross the DataLoader worker-to-main boundary, and rebuilds them in `transfer_batch_to_device`. Every tensor a worker returns is moved into its own shared-memory segment and passed to the parent as a file descriptor, so a batch of 16 crosses as 114 objects of which 112 carry a few kilobytes in total; packing takes that to 9 without changing a byte of payload. The rebuilt targets are bit-identical, dtypes included, and the training step receives the same plain list of dicts as before. Off by default: the loaders of a `RFDETRDataModule` built with it on yield a `PackedTargets` sequence rather than a tuple of dicts whenever a batch can be packed losslessly, which is visible to callers that consume `train_dataloader()` directly; a batch that cannot (mismatched keys, a non-tensor value, or dtypes that would be silently promoted by `torch.cat`) falls back to the original tuple of dicts.
