...
- `SetCriterion.loss_masks` now samples matched ground-truth mask labels via direct tensor indexing instead of `point_sample`'s grid-sample machinery, guarded to the case where the matched masks are large enough (at least 65536 total elements per image group, at least 2 matches per image group, and at least 16x the number of points being sampled in aggregate) and both the masks and point coordinates sit on CPU or the same CUDA device, with float32 point coordinates and 1-D int64 CPU-side matcher indices; anything outside that contract keeps using the previous `point_sample` path unchanged, and the sampled labels are bit-identical either way. The per-group match-count floor exists because a single large mask can clear the element floor on its own while carrying only one match — the fixed per-image loop overhead (slicing, a device transfer, a gather) then dominates and is a stable ~1.25-1.5x regression, independent of mask resolution or number of images. Measured on an i9-13900HX (single thread, 8 ground-truth masks x 13 repeated match groups, 312x312 masks, 380 sampled points, median of 100 calls, 7 repeated trials): the sampling step itself is 68.7x-75.9x faster and `loss_masks` end-to-end is 7.2x-7.4x faster, with 93.255% less CPU allocation traffic recorded by the profiler for the sampling step.
