...
- `TrainConfig.eval_batch_size` decouples the validation/test/predict dataloaders from the training `batch_size`. Default `None` inherits `batch_size` as before; unlike `batch_size` it accepts no `"auto"`. ([#1378](https://github.com/roboflow/rf-detr/pull/1378))
...
- `RFDETR.predict()` performance work, none of it changing detections: every entry measured byte-identical or checksum-identical against the previous path.
    ...
    - Transfers PIL/uint8 NumPy inputs to device in their original byte storage and widens to float on-device instead of on host, cutting host-to-device transfer size 4x. ([#1415](https://github.com/roboflow/rf-detr/pull/1415))
    - Converts PIL/uint8 NumPy inputs to contiguous CHW float storage in one fused allocation instead of a separate dtype/layout pass. ([#1390](https://github.com/roboflow/rf-detr/pull/1390))
    - `include_source_image=True` converts CUDA float images to `uint8` source bytes on-device before the host transfer instead of casting on CPU. CPU tensors and unsupported CUDA dtypes (e.g. `bfloat16`) keep the previous path. ([#1388](https://github.com/roboflow/rf-detr/pull/1388))
...
- Single-feature-level fast paths reuse tensors instead of re-materializing them (current Nano/Small/Medium/Large models; legacy `RFDETRLargeDeprecatedConfig` unaffected where noted); outputs bit-identical:
    ...
    - Deformable attention reuses its sampled tensor directly for single-level inputs instead of stack+flatten over a one-element list; this mainly benefits keypoint cross-attention. ([#1385](https://github.com/roboflow/rf-detr/pull/1385))
    ...
    - Decoder's grouped self-attention reuses the regrouped query tensor as the key instead of materializing the same grouping twice. ([#1371](https://github.com/roboflow/rf-detr/pull/1371))
- Evaluation:
...
- Segmentation postprocessing, both bit-identical to the previous output:
    - Reads each image's mask resize target once per batch instead of per image, cutting CUDA syncs; same fix applied to `COCOEvalCallback._convert_targets`. ([#1369](https://github.com/roboflow/rf-detr/pull/1369))
    - Writes thresholded interpolation chunks directly into a preallocated buffer instead of `torch.cat`-ing a list. Small CUDA selections keep the prior path (lower peak memory at shipped `num_select=100` defaults). ([#1374](https://github.com/roboflow/rf-detr/pull/1374))
- `SetCriterion.loss_masks` samples matched ground-truth mask labels via direct tensor indexing instead of `point_sample`, under size/contiguity/dtype guards (CUDA keeps the previous path); measured 6.7-7.1x faster on a single-thread CPU microbenchmark of the full `loss_masks` call, bit-identical labels either way. ([#1367](https://github.com/roboflow/rf-detr/pull/1367))
...
- Training skips PyTorch Lightning's pre-training sanity validation batches by default (`num_sanity_val_steps` restores it). Per-microbatch training-loss metrics are compacted (17 → 9 keys on default `RFDETRSmall`; `compact_train_metrics=False` restores per-layer keys). LR metrics now emit only on optimizer updates instead of every microbatch: a no-op at the new `grad_accum_steps=1` default, but ~75% fewer log calls at `grad_accum_steps=4` (the 1.9.x default). ([#1360](https://github.com/roboflow/rf-detr/pull/1360))
...
- Packed targets now materialize directly into per-sample device tensors instead of clone-after-move, removing a transient CUDA allocation equal to the mask field's size. ([#1405](https://github.com/roboflow/rf-detr/pull/1405))
...
- `TrainConfig.log_per_class_metrics` now defaults `False` (was `True`), so per-class AP keys are no longer emitted by default. `TrainConfig.compute_val_loss` now defaults `"auto"` (was `True`), so `val/loss` is computed only when a scheduler/callback actually consumes it. Set either explicitly to restore the prior unconditional behavior. ([#1372](https://github.com/roboflow/rf-detr/pull/1372))
