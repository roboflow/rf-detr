...
- `TrainConfig.optimizer` (`str | Callable`) and `optimizer_kwargs` — configurable training optimizer. `optimizer="adamw"` (the default) keeps RF-DETR's built-in fused `torch.optim.AdamW` path unchanged. A short name is loaded from the new [`pytorch-optimizer`](https://github.com/kozistr/pytorch_optimizer) dependency (in the `[train]` extra), e.g. `optimizer="lion"`; a dotted import path (`"torch.optim.AdamW"`) or a callable / `functools.partial` selects an optimizer explicitly and is called with the RF-DETR parameter groups. `optimizer_kwargs` forwards constructor arguments (ignored for callables, which bake their own arguments in). ([#1006](https://github.com/roboflow/rf-detr/pull/1006))
