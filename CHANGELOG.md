...
- Segmentation postprocessing now writes each thresholded interpolation chunk directly into a preallocated destination tensor instead of collecting bool chunks in a list for a final `torch.cat`, removing the previous CPU-only device gate (verified on CPU and CUDA; MPS and XLA were not exercised). Bit-identical to the previous output. An earlier version of this change was CPU-only: a plain `for` loop reassigning the same `interpolated` variable each iteration keeps the *previous* chunk's transient float32 buffer referenced while `F.interpolate` builds the *next* one (Python evaluates the new value before rebinding the name), so on CUDA two chunks briefly coexisted instead of one — the same double-buffering this change exists to remove, just smaller and inside this loop. An explicit `del interpolated` after each chunk is consumed removes it. Measured on an i9-13900HX (CPU, single thread, median of 11 repeated trials with min-max range recorded) and an RTX 4060 Laptop GPU (CUDA, median of 15 repeated trials): at the largest realistic case (K=300, 192x192 masks upsampled to 1920x1080, ~622 MB bool output), CPU latency dropped ~12% and fresh-process peak RSS dropped by ~360 MiB (~21%); CUDA latency dropped ~17% and CUDA peak allocated memory dropped ~29%. CUDA memory behaviour is chunk-count dependent, root-caused with a step-by-step `torch.cuda.memory_stats()` trace and confirmed with a controlled sweep that holds output size fixed while varying chunk count: this path's CUDA peak is always "full output plus at most one transient chunk", while the old path's peak is the larger of a mid-loop moment (partial output plus one transient chunk) or the final `torch.cat`'s full doubling. With few chunks (`num_select` at or below `4x _MASK_CHUNK=128`, e.g. `RFDETRSegNano`/`Small`'s default `num_select=100` -> 4 chunks) the old path's mid-loop peak can still land a few percent below this path's flat one (measured +1.8% to +3.75% on a real `RFDETRSegNano` forward pass, 2,091,271,680 vs 2,015,774,208 bytes, reproducible); from `num_select=200` (`RFDETRSegMedium`/`Large`, 7 chunks) this path already wins CUDA memory too (-17.9%), improving further at `num_select=300` (`RFDETRSegXLarge`/`2XLarge`, 10 chunks, -28.5%). `_MASK_CHUNK` itself is left untuned here — a smaller chunk size could push the `num_select=100` configs into the same winning regime, but it also trades off against Python loop and kernel-launch overhead, a separate axis from what this change addresses. No output size or `num_select` regressed in latency on either device; the CUDA memory increase at `num_select<=100` is the one open, now fully understood, cost of this change.
