...
- Fix detection head reinitialization logic when loading a fine-tuned checkpoint that has fewer classes than the model's default `num_classes`. Previously the second `reinitialize_detection_head` call would resize the head to `num_classes + 1` after `load_state_dict`, destroying the loaded fine-tuned weights. The second reinit now only fires in the backbone-pretrain scenario (checkpoint has *more* classes than configured), preserving loaded weights for fine-tuned checkpoints ([#815](https://github.com/roboflow/rf-detr/pull/815), closes [#813](https://github.com/roboflow/rf-detr/issues/813)).
