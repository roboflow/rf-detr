...
- COCO mAP computation now converts each image's detection scores once instead of once per detection. TorchMetrics' `_get_coco_format` hoists boxes and labels to Python lists per image but reads scores one annotation at a time (`scores[image_id][k].cpu().tolist()`), which is a tensor index, a device copy, and a scalar conversion for every detection — roughly 593k of them per `compute()` at a 1978-image validation split with `num_queries=300`, and `compute()` runs for both the base and EMA model. `OnePassCocoMeanAveragePrecision` now asks TorchMetrics for the same prediction annotations with `scores=None` and assigns per-image score lists onto them afterwards, so every other upstream branch (segmentation areas, crowd flags, the `info` key, per-annotation validation) is still upstream's. Datasets are byte-identical, verified over 593,400 synthetic detections; the construction step measured 1.56x faster (the annotation loop alone, 1.68x) on one x86 workstation with synthetic state — a ratio on host-side conversion, not an end-to-end training measurement, and not transferable as seconds to other hardware. A prediction state with no boxes at all (mask-only segmentation) keeps using TorchMetrics' helper unchanged, because upstream can then drop whole images from the annotation list and positional score assignment would no longer line up. A prediction annotation count that stops matching the stored score count now raises instead of silently mis-assigning scores.
