...
- TPU/XLA EMA updates now keep their control-flow counter on the host, run inside the optimizer step, and place `xm.optimization_barrier_()` over the model's parameters and buffers before averaging, without adding another per-step synchronization. This prevents the deleted-buffer failure seen after a few optimizer steps, including on four TPU cores under BF16. CPU and CUDA are unchanged. ([#1058](https://github.com/roboflow/rf-detr/issues/1058))
