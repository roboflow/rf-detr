...
- Validation now evaluates **one** model per epoch instead of two: the EMA-averaged weights when `use_ema=True` (the default), the base weights otherwise. The base-model forward pass that `COCOEvalCallback` used to run over every validation batch is gone, which removes one full pass over the validation set per epoch — measured at roughly 3-3.5% of epoch wall time on an L4 run with a 1978-image validation split (11.32 s of 124 EMA forwards against a 222 s epoch); the fraction depends on the validation-to-training ratio of a given dataset and is not a fixed figure. New `TrainConfig.eval_base_model` (default `False`) restores the previous base+EMA comparison. `TrainConfig.eval_ema_only` is deprecated (removal in v2.0): it is now a no-op alias for the default and emits a `FutureWarning`; it still requires `use_ema=True` and is rejected together with `eval_base_model=True`.

    **Metric keys move.** `val/mAP_50_95`, `val/mAP_50` and `val/mAR` now report whichever model was evaluated first-class — the EMA model on a default run, the base model under `eval_base_model=True` — mirrored from the EMA track so schedulers, early-stopping hooks, checkpoint monitors and dashboards watching the primary key keep receiving a real number rather than nothing (the gap that made the old `eval_ema_only` flag unsuitable as a default). `val/ema_*` is unchanged and still feeds `monitor_ema`. Two keys are deliberately not mirrored: per-class AP stays at `val/ema_AP/<class>`, and `val/mAP_75` is not emitted when only the EMA track ran. `val/F1` continues to follow `validation_step`'s own forward, which under the default is the same EMA model the mirrored mAP reports — so the two now agree, where previously `val/F1` reported EMA quality alongside a base-model mAP. `val/loss` and its per-term components follow that same forward: on a default run they are now computed from the EMA weights rather than the base weights, so a `ReduceLROnPlateau` scheduler, `ModelCheckpoint(monitor="val/loss")` or early stopping watching that key tracks the EMA model from this release on.
