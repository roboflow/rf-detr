...
- `CocoDetection` and `YoloDetection` now decode JPEG files with `simplejpeg` (libjpeg-turbo straight into a NumPy buffer) when it is installed, which the `[train]` extra now includes. Single-worker decode of a 640x480 JPEG went from 2.7 ms to 1.7 ms including the conversion back to the PIL image the COCO transform pipeline expects (1920x1440: 19.4 ms to 11.6 ms; with `draft_size` reduction 6.5 ms to 4.2 ms), and the pixels are identical to Pillow's because Pillow's wheels link the same libjpeg-turbo. Non-JPEG files, an environment without `simplejpeg`, and JPEGs it rejects still decode through Pillow, with the same errors as before. `draft_size` applies the same power-of-two reduction on both decoders. ([#1391](https://github.com/roboflow/rf-detr/issues/1391))
...
- Validation now evaluates **one** model per epoch instead of two: the EMA-averaged weights when `use_ema=True` (the default), the base weights otherwise. The base-model forward pass that `COCOEvalCallback` used to run over every validation batch is gone, which removes one full pass over the validation set per epoch — measured at 11.32 s in one L4 run with a 1978-image validation split and a 222 s epoch, approximately 5.1% in that run; the fraction depends on the validation-to-training ratio and is not a fixed figure. New `TrainConfig.eval_base_model` (default `False`) restores the previous base+EMA comparison. `TrainConfig.eval_ema_only` is deprecated (removal in v1.13): explicit legacy `True` preserves EMA-only evaluation and explicit legacy `False` preserves base-plus-EMA evaluation, with a `FutureWarning`; omit it from new configurations. It still requires `use_ema=True` when true and conflicts with `eval_base_model=True`.

    **Metric keys move.** `val/mAP_50_95`, `val/mAP_50`, `val/mAP_75`, `val/mAR`, and per-class `val/AP/<class>` now report whichever model was evaluated first-class — the EMA model on a default run, the base model under `eval_base_model=True` — while `val/ema_*` and `val/ema_AP/<class>` remain available for explicit EMA consumers. Aggregate keys are mirrored from the EMA track so schedulers, early-stopping hooks, checkpoint monitors and dashboards watching the primary namespace keep receiving a real number rather than nothing (the gap that made the old `eval_ema_only` flag unsuitable as a default). `val/F1` continues to follow `validation_step`'s own forward, which under the default is the same EMA model the mirrored mAP reports — so the two now agree, where previously `val/F1` reported EMA quality alongside a base-model mAP. `val/loss` and its per-term components follow that same forward: on a default run they are now computed from the EMA weights rather than the base weights, so a `ReduceLROnPlateau` scheduler, `ModelCheckpoint(monitor="val/loss")` or early stopping watching that key tracks the EMA model from this release on.

    **Checkpointing.** When the base model is not evaluated, `BestModelCallback`'s regular track is disabled rather than checkpointing base weights against an EMA score; `checkpoint_best_ema.pth` is promoted to `checkpoint_best_total.pth` as before, and `checkpoint_best_regular.pth` is not written. `use_ema=False` runs are unaffected in every respect.
...
- Segmentation postprocessing now writes thresholded interpolation chunks directly into a preallocated destination on CPU and larger CUDA selections instead of collecting bool chunks for a final `torch.cat`; output values remain bit-identical. CUDA selections up to four `_MASK_CHUNK` batches retain the previous list/`torch.cat` path because the documented measurements show that its partial-output peak is lower for the shipped `num_select=100` SegNano/SegSmall defaults. Backends without verified `torch.gt(..., out=...)` support, including XLA and MPS in this change, use the established path. Reproduce the K={100,200,300} comparison with `benchmarks/postprocess_mask_memory.py`; the committed harness reports synchronization, environment, output checksum, and peak-memory measurement method.

- `SetCriterion.loss_masks` now samples matched ground-truth mask labels via direct tensor indexing instead of `point_sample`'s grid-sample machinery on CPU, guarded to the case where the matched masks are large enough (at least 1048576 total elements per image group, at least 2 matches per image group, and at least 16x the number of points being sampled in aggregate), contiguous with a shared spatial shape, and paired with float32 point coordinates plus 1-D int64 CPU-side matcher indices whose values are in range; CUDA and anything else outside that contract keeps using the previous `point_sample` path unchanged, and the sampled labels are bit-identical either way. The per-group match-count floor exists because a single large mask can clear the element floor on its own while carrying only one match — the fixed per-image loop overhead (slicing, a device transfer, a gather) then dominates regardless of resolution. These routing floors are conservative bounds, not calibrated crossover points. Measured on an i9-13900HX (single thread, 8 ground-truth masks x 13 repeated match groups, 312x312 masks, 380 sampled points, median of 100 calls, 7 repeated trials): the sampling step itself is 50.8x-58.4x faster and `loss_masks` end-to-end is 6.7x-7.1x faster, with 92.034% less CPU allocation traffic recorded by the profiler for the sampling step.
...
- Detection validation now converts each batch's ground-truth targets once and hands the same converted list to both the base-model and the EMA mAP accumulator, instead of repeating an identical conversion for the second one. Only segmentation feeds predictions into the conversion, so outside it the two calls rescaled the same boxes to absolute xyxy and repeated the same `orig_size` device-to-host transfer for no difference in either metric. Segmentation still converts twice on purpose: target masks are resampled onto each paired prediction's own mask grid, and the base and EMA heads can emit different grids for the same image. Reuse is safe because both accumulators store detached CPU copies of what they are handed rather than retaining the caller's dicts. What this removes is one target-conversion pass, including active-device box conversion and one `orig_size` device-to-host synchronization, while model-forward GPU compute remains unchanged; it applies only to runs that evaluate both models.

- `RFDETR.predict()`'s default `include_source_image=True` path now converts a normalized CUDA `float16`/`float32`/`float64` image to its `uint8` source-metadata bytes on-device before the host transfer, instead of transferring the floating-point tensor and casting on the CPU. CPU-tensor inputs, CUDA tensors of another dtype (e.g. `bfloat16`, unsupported by this conversion on both the previous and the current path), and degenerate single-pixel-dimension tensors keep the previous NumPy-side conversion. Returned bytes, dtype, channel-major strides, ownership, and writeability are unchanged; a CUDA-tensor input with out-of-range pixel values continues to raise the same public `ValueError` before any converted array is returned. A NaN pixel still converts successfully, but the previous NumPy-side cast's incidental invalid-cast `RuntimeWarning` is not reproduced on the on-device path. The on-device cast allocates two short-lived same-shape CUDA temporaries (the scaled float image, then its `uint8` cast) ahead of the transfer; measured on an RTX 4060 Laptop GPU, the extra peak allocation over the previous (host-only) path is 4.7 MB for a 560x560 `float32` image and 24.6 MB at 1280x1280, freed immediately after the per-image conversion and well before the batch forward pass.
