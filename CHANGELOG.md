...
- TFLite reference inference helper no longer assumes that a lone rank-4 output is a segmentation mask. The ONNX output names rarely survive the conversion — RF-DETR's own TFLite files arrive as `StatefulPartitionedCall:N` — so a keypoint export's `pred_keypoints` tensor was indistinguishable from a mask by name and was silently upsampled into `Detections.mask`. The kind is now taken from an explicit `rank4_output` argument (`"masks"` by default, so segmentation exports decode exactly as before).
