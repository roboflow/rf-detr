...
- `TrainConfig.optimizer`, `optimizer_kwargs`, and `optimizer_param_group_overrides` — configurable training optimizer. `optimizer="adamw"` (the default) keeps RF-DETR's built-in fused `torch.optim.AdamW` path unchanged; any other name is loaded from the new [`pytorch-optimizer`](https://github.com/kozistr/pytorch_optimizer) dependency (in the `[train]` extra), for example `optimizer="lion"`. `optimizer_kwargs` forwards constructor arguments to the selected optimizer, and `optimizer_param_group_overrides` applies rank-based kwargs for hybrid optimizers on non-default optimizers. ([#1006](https://github.com/roboflow/rf-detr/pull/1006))
