...
This page describes how the export pipeline is assembled and what it takes to add a format to it. If you only want to export a model, read [Export RF-DETR Model](../learn/export.md) instead — everything here is internal API.
...
Add it to `RFDETR.export()`'s `format` docstring in `src/rfdetr/detr.py` and to [Export RF-DETR Model](../learn/export.md) — installation extra, a basic example, output files, and an inference snippet.
...
- [Export RF-DETR Model](../learn/export.md) — the user-facing export guide
