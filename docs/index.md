...
> **Benchmarking note:** Current `RFDETRLarge` defaults to `rf-detr-large-2026.pth`. The older `rf-detr-large.pth` checkpoint is a legacy Large release kept for backward compatibility, and it is both slower and less accurate than the current Large model. If you benchmark exported ONNX/TensorRT artifacts, verify which Large checkpoint the graph came from before comparing latency.
