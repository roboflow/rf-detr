...
!!! warning "Breaking: the export internals are now exporter classes"
    ...
    Every path with a leading underscore was already internal and carried no stability guarantee; they are listed so the move is discoverable rather than a silent break. Prefer `RFDETR.export(format=...)`. The contract each exporter implements is documented in the [Exporter Blueprint](../exports/export-blueprint.md).
