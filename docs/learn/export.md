...
!!! warning "Experimental — Use with Caution"
    ...
    **Known limitations:**
    ...
    - `litert-torch` 0.9.x pins `torch<2.14` in its own package metadata, so `pip install "rfdetr[litert]"` resolves an older torch than a plain `rfdetr` install on Python environments that would otherwise get 2.14+. `[litert]` also cannot share an environment with the `[tflite]` extra (conflicting `ai-edge-litert` pins) or with the mypy `typing` dependency group — install it on its own, or expect `uv`/`pip` to report a resolution conflict.
