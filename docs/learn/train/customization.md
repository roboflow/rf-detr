...
| Mixed precision       | `train_config.amp_dtype` controls AMP (`None` disables it, `"auto"` selects `bf16-mixed` on Ampere+, `"bf16"` / `"fp16"` force a specific dtype, and `"fp8"` selects Lightning Transformer Engine on supported NVIDIA GPUs); the `model_config.amp` boolean is deprecated |
