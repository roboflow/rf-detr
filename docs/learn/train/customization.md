...
| Mixed precision       | `model_config.amp` enables precision selection; `train_config.amp_dtype` controls the dtype (`"auto"` and `"bf16"` select XLA's `bf16-true` on TPU/XLA; elsewhere, `"auto"` selects `bf16-mixed` on Ampere+ CUDA; `"fp16"` selects fp16 on CUDA/MPS but resolves to FP32 on TPU/XLA:CPU; `"fp8"` selects Lightning Transformer Engine on supported NVIDIA GPUs) |
