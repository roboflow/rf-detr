...
| Mixed precision       | `train_config.amp_dtype` controls AMP (`None` disables it; `"auto"` and `"bf16"` select XLA's `bf16-true` on TPU/XLA; elsewhere, `"auto"` selects `bf16-mixed` on Ampere+ CUDA; `"fp16"` selects fp16 on CUDA/MPS but resolves to FP32 on TPU/XLA/CPU; and `"fp8"` selects Lightning Transformer Engine on supported NVIDIA GPUs). The constructor-only `model_config.amp` boolean is deprecated and is consulted only when `amp_dtype` remains at its default `"auto"`. |
