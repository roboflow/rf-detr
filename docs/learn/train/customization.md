...
| Mixed precision       | `model_config.amp` enables AMP; dtype resolved from `train_config.amp_dtype` (`"auto"` and `"bf16"` select XLA's `bf16-true` on TPU/XLA, `bf16-mixed` on Ampere+ CUDA otherwise; `"fp16"` forces fp16; `"fp8"` selects Lightning Transformer Engine on supported NVIDIA GPUs) |
