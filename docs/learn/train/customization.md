...
| `transfer_batch_to_device` | Moves `NestedTensor` batches to the target device. When `TrainConfig.pack_targets=True`, also unpacks the collated `PackedTargets` back into the plain per-sample dict list.                                                    |
