...
| `coco_backend`               | `str`               | `"faster_coco_eval"` | COCO bbox/mask evaluator: `"faster_coco_eval"` or `"ultrafast"`. The latter requires `pip install "rfdetr[ultrafast]"`.                                                                                                                                              |
...
Install `pip install "rfdetr[ultrafast]"`, then select the backend for training validation or standalone evaluation:

```python
from rfdetr import RFDETRSmall

model = RFDETRSmall()
model.train(dataset_dir="path/to/dataset", coco_backend="ultrafast")
# Or run standalone evaluation:
model.evaluate(dataset_dir="path/to/dataset", coco_backend="ultrafast")
```

The selection applies to bbox/mask metrics for the regular, train-split, and EMA tracks. The default evaluator remains `faster_coco_eval`. RF-DETR's custom keypoint OKS and F1 metrics use their existing implementations. Both backends use the same `eval_max_dets` and per-class settings.
