...
| `eval_backend`               | `Literal["hotcoco","faster_coco_eval","ufcoco","vernier"]` | `"vernier"` | COCO evaluation backend for detection and segmentation mAP. All four ship with `rfdetr[train]` and return identical metrics; `"vernier"` additionally requires a box on every annotation, so it cannot evaluate a mask-only run. `"faster_coco_eval"` selects the previous, slower evaluator, `"ufcoco"` selects ultrafast-pycocotools and `"hotcoco"` selects hotcoco. |
...
- `eval_backend` selects the COCO evaluator. The default `"vernier"` ([vernier](https://github.com/NoeFontana/vernier)) ships with `rfdetr[train]`; `"faster_coco_eval"` restores the previous evaluator. Metrics are identical, not merely close: the parity tests compare every aggregate, per-class and class-ID output of every backend for box-only and box-plus-mask evaluation and require exact equality. The knob changes the cost of `compute()`, not the validation forward pass that usually dominates a validation epoch, so it matters most on large validation sets. `"vernier"` is the one backend that requires a box on every annotation, so it cannot evaluate a mask-only run: an `iou_type` without `"bbox"` is rejected when the metric is constructed rather than at `compute()`, so the run fails immediately instead of after a validation epoch. On a synthetic COCO-val-shaped state (5,000 images, 300 detections per image, 80 classes, box-only) one Linux four-core `compute()` took 3.18 s on `hotcoco` against 1.86 s on `vernier`, a ~1.7x speedup; the macOS-CPU figures below were measured on a different machine and are not directly comparable to these. The margin is wider away from that shape: on the 20-class synthetic configuration `vernier` is 2.1-2.3x `hotcoco` on box-only and 5.9-8.6x on box-plus-mask evaluation, at every core count from one to eight. These are single-process timings; under multi-GPU DDP each rank merges the gathered state and runs the whole `compute()` itself, so the ranks evaluate concurrently and the wall-clock saving per validation epoch may differ. Keypoint evaluation uses its own OKS path and is unaffected, as is the ONNX/TensorRT benchmark evaluator.
- `eval_backend="hotcoco"` selects [hotcoco](https://github.com/derekallman/hotcoco), a Rust COCO evaluator. On synthetic COCO-val-shaped state (5,000 images, 36.6k ground-truth boxes, 300 detections per image, 80 classes, `eval_max_dets=500`) one macOS-CPU `compute()` took 6.2 s on `faster_coco_eval` against 1.1 s on `hotcoco`. It ships with `rfdetr[train]` like the other backends.
- `eval_backend="ufcoco"` selects [ultrafast-pycocotools](https://github.com/developer0hye/ultrafast-pycocotools), a Rust evaluator that reproduces pycocotools' precision, recall and score arrays byte for byte. It ships with `rfdetr[train]` like the other backends; pass `eval_backend="ufcoco"` to `model.train(...)` or `model.evaluate(...)`. The selection reaches the validation, train-split and EMA metrics alike, and the parity tests hold it to the same exact equality against `faster_coco_eval` as `hotcoco`, for box-only and box-plus-mask evaluation at `eval_max_dets` 100 and 500.
- Adding a new `eval_backend` or benchmarking an existing one: see [Advanced Training → COCO Evaluation Backends](advanced.md#coco-evaluation-backends).
...
| `lr_scheduler_kwargs`   | `dict`            | `{}`         | Keyword arguments forwarded to an explicit scheduler; for the managed presets carries `lr_drop` (default 100, epoch of the 10x step drop) and `min_factor` (default 0.0, cosine floor as a fraction of the initial LR). |
...
| `eval_backend`               | `Literal["hotcoco","faster_coco_eval","ufcoco","vernier"]` | "vernier"      | COCO evaluation backend; `"faster_coco_eval"` selects the previous, slower evaluator, `"ufcoco"` for ultrafast-pycocotools and `"hotcoco"` for hotcoco.                                                                   |
...
| `lr_scheduler_kwargs`        | dict                                                       | {}             | Keyword arguments for an explicit scheduler; for the managed presets carries lr_drop (default 100, epoch of the 10x step drop) and min_factor (default 0.0, cosine floor).                                                |
