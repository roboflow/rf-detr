...
- `eval_backend` selects the COCO evaluator. The default `"vernier"` ([vernier](https://github.com/NoeFontana/vernier)) ships with `rfdetr[train]`; `"faster_coco_eval"` restores the previous evaluator. Metrics are identical, not merely close: the parity tests compare every aggregate, per-class and class-ID output of every backend for box-only and box-plus-mask evaluation and require exact equality. The knob changes the cost of `compute()`, not the validation forward pass that usually dominates a validation epoch, so it matters most on large validation sets. Keypoint evaluation uses its own OKS path and is unaffected, as is the ONNX/TensorRT benchmark evaluator.
