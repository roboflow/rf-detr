...
| `cuda_graphs`            | `bool` | `False` | **Constructor-only parameter.** Capture and replay the detection training forward on one CUDA GPU with BF16 precision. Each input shape pays a first-use capture cost and retains graph memory. Unsupported configurations use eager training with a warning. Combining with `compile=True` raises; capture errors stop training and require a process restart before retrying eagerly. See [Advanced Training](advanced.md#cuda-graph-training). |
