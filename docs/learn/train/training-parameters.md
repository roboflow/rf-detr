...
| `eval_ema_only`   | `bool` | `False` | **Deprecated (removal in v2.0)** — evaluating only the EMA model is now the default, so this flag is a no-op that emits a `FutureWarning`. Use `eval_base_model=True` to opt back in to the base-model pass.                                                    |
...
| `eval_ema_only`              | `bool`              | `False` | **Deprecated (removal in v2.0)** — no-op alias for the default policy; emits a `FutureWarning`. Use `eval_base_model`.                                                                                                                                               |
...
- `eval_base_model=False` is the default: validation runs **one** forward pass per batch, through the EMA weights when `use_ema=True` and through the base weights otherwise. This removes a full forward pass over the validation set each epoch. `val/mAP_*` reports the model that was evaluated — the EMA model under the default — so schedulers, early stopping, checkpoint monitors and dashboards keep receiving a real number; `val/ema_*` still carries the EMA track for `monitor_ema`. Two keys stay EMA-only: per-class AP (`val/ema_AP/<class>`) and `val/mAP_75`. The best-checkpoint "regular" track is disabled in this mode, because it saves base weights that were never scored; `checkpoint_best_ema.pth` is promoted to `checkpoint_best_total.pth` instead.
...
| `eval_ema_only`              | bool                   | False          | Deprecated (removal in v2.0); no-op alias for the default policy. Use `eval_base_model`.                                                                                                                                  |
