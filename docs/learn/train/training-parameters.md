...
| `pack_targets`       | `bool` | `False` | Concatenate each batch's per-sample target dicts into one tensor per field before crossing the DataLoader worker-to-main boundary, cutting per-batch object count. `train_dataloader()` yields a `PackedTargets` sequence instead of a tuple of dicts whenever a batch packs losslessly. |
