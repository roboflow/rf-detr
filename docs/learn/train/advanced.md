...
!!! tip "aug_config={} also disables the resize-crop branch"

    Passing `aug_config={}` disables both the Albumentations augmentation stack **and** the
    resize → crop → resize branch (Option B) in the training pipeline.
    Annotations near image borders will not be clipped.
    Pass `aug_config=None` to keep the default augmentation stack with the crop branch active.
