...
| `key_points.class_id`             | `(N,)`         | Model label ID for each detection. COCO-pretrained checkpoints use sparse COCO category IDs (1–90). Fine-tuned keypoint checkpoints use a shifted scheme: slot 0 is background (`"__background__"`), foreground classes start at slot 1. Use `key_points.data["class_name"]` for name resolution rather than indexing your class list by `class_id`. |
